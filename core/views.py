...
def actualizar_estado_pedido(request, pedido_id):
    ...
    if request.method == 'POST':
        ...
        if nuevo_estado in dict(Pedido.ESTADO_PEDIDO).keys():
            pedido.estado = nuevo_estado
            pedido.save()
            return redirect('historial_pedidos_admin')

    return render(request, 'actualizar_estado_pedido.html', {'pedido': pedido})

def historial_compras(request):
    if not request.user.is_authenticated:
        return redirect('login')

    pedidos = Pedido.objects.filter(usuario=request.user).order_by('-fecha_actual')
    return render(request, 'historial_compras.html', {'pedidos': pedidos})
...
def confirmar_compra(request):
    ...
    codigo_aleatorio = ''.join(random.choices(string.ascii_uppercase + string.digits, k=8))
    numero_seguimiento = ''.join(random.choices(string.ascii_uppercase + string.digits, k=12))  # Generar número de seguimiento
    ...
    pedido = Pedido.objects.create(
        ...
        estado='pendiente',
        numero_seguimiento=numero_seguimiento  # Asignar número de seguimiento
    )
    ...
    request.session['fecha_compra'] = fecha_actual.strftime("%d/%m/%Y %H:%M:%S")
    request.session['codigo_pedido'] = codigo_aleatorio

    return redirect('pagina_de_gracias')

def pagina_de_gracias(request):
    codigo_pedido = request.session.get('codigo_pedido')
    fecha_compra = request.session.get('fecha_compra')
    return render(request, 'gracias.html', {'codigo_aleatorio': codigo_pedido, 'fecha_actual': fecha_compra})

    if not fecha_compra:
        # Usa la hora actual en UTC y luego conviértela a la zona horaria configurada
        fecha_compra = timezone.localtime(timezone.now()).strftime("%d/%m/%Y %H:%M:%S")
    
    if not codigo_pedido:
        codigo_pedido = ''.join(random.choices(string.ascii_uppercase + string.digits, k=8))
    
    # Eliminar la información de la sesión para evitar su reutilización
    request.session.pop('fecha_compra', None)
    request.session.pop('codigo_pedido', None)
    
    context = {
        'fecha_actual': fecha_compra,
        'codigo_aleatorio': codigo_pedido,
    }
